...
String  url = "https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey=qO3Yw24kVkepMeKVkhXkIA&senderid=CARTCN&channel=2&DCS=0&flashsms=0&number=91"+prefManager.getMobileNumber()+"&text=Cart Corner has received Your Order and Cartcorner representative will deliver  by "+deliverydate+" - "+deliverytime +"\n"+ "Thank You&route=0";
    ...
    private void sendEmail() {
        ...
        stringBuffer.append("Hi "+ prefManager.getName() +" Your order Has been Placed Successfully and CartCorner will delivery on "+deliverydate +"-"+deliverytime +"\n\n");
        stringBuffer.append("Product Details" +"\n");
        for (int i=0;i<cartItems.size();i++){
            stringBuffer.append(cartItems.get(i).getProductname() +" : "+cartItems.get(i).getProductquno()+"*"+cartItems.get(i).getProductweight() +" - "+ cartItems.get(i).getProductdisprice() + "\n");
        }
        ...
        stringBuffer.append("Name : "+ prefManager.getName() +"\n");
        stringBuffer.append("Email : "+ prefManager.getEmail() +"\n");
        stringBuffer.append("Mobile Number : "+ prefManager.getMobileNumber() +"\n");
        stringBuffer.append("Address : "+ deliveryaddresscheckout.getText().toString());
